...
❓ Why does Java use Marker Interfaces?
...
🧠 Key Points to Remember (Interview Notes)
...
Try lock					❌			✅ tryLock() avoids blocking
...
🔹 Key Properties
	...
	•	Explicit unlock: Must call unlock() in finally block
...
🔹 1. Future
	...
	•	Introduced in Java 5 (java.util.concurrent.Future).
	•	Represents the result of an asynchronous computation.
	•	Can get the result using get(), which blocks the thread until the computation is done.
	•	Cannot easily chain multiple tasks or handle callbacks without extra boilerplate.
...
Limitations of Future:
	...
	•	Blocking get()
	•	No easy way to chain tasks (thenApply)
	•	Cannot complete manually (only by executor)

⸻

🔹 2. CompletableFuture

	•	Introduced in Java 8 (java.util.concurrent.CompletableFuture).
	•	Extends Future with non-blocking, async capabilities.
	•	Supports:
	•	Chaining (thenApply, thenAccept)
	•	Combining multiple futures (thenCombine, allOf, anyOf)
	•	Exception handling (exceptionally, handle)
	•	Can be completed manually using complete().
