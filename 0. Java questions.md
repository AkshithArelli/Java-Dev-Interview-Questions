...
Anonymous Class Example 1 — Extending a Class
```java
class Animal {
    void sound() {
...
Anonymous Class Example 2 — Implementing Interface
```java
interface Calculator {
    int add(int a, int b);
}
...
✅ 4. Inner Class (Non-static inner class)
...
In Java, “nested class” = inner class + static inner class.
But if interview asks separately, it means:

“How to create an instance of a class inside another class?”
...
👉 It checks whether two references (memory addresses) point to the same object.

It’s like asking:

“Are these two people the exact same person?”
...
== compares actual values, because they’re not objects.
...
```
👉 Here both strings have same content but are different objects in memory,
...
🧩 2️⃣ What is .equals() in Java?

.equals() is a method defined in the Object class. (not Objects)

Classes can override it to define what “equality” means for them.
...
Example 1 — Default behavior (from Object)

### If you don’t override .equals(), it behaves like ==:
```java
class Person {
    String name;
...
### Example 2 — Overridden behavior (in String, Integer, etc.)
```java
...
🧱 3️⃣ Summary: == vs .equals()
```
Comparison		Works On							Checks													Overridable?		Example
		
==				Primitives & Object References		For primitives: value  For objects: memory address		❌ No				s1 == s2

.equals()		Only Objects						Content (if overridden), else memory					✅ Yes				s1.equals(s2)
```
⸻
...
Integer i1 = 100;
Integer i2 = 100;
System.out.println(i1 == i2);       // ✅ true (cached values from -128 to 127)
...
System.out.println(c1.equals(c2));  // ❌ false (not overridden)
```
...
🧭 Final 3-Line Summary (Easy to Remember)
...
### 🏠 1. Hash Family → hashCode + equals

Anything with “Hash” in its name

(HashSet, HashMap, LinkedHashMap)

✅ Uses hashCode + equals to find and compare elements.
...
### 🌲 2. Tree Family → compareTo / Comparator

Anything with “Tree” in its name

(TreeSet, TreeMap)

✅ Uses compareTo() or Comparator.compare()

for equality and ordering.

💬 Memory Trick:

“Tree compares to grow in order.”

⸻

### 📦 3. List Family → equals() only

(ArrayList, LinkedList)

✅ Uses equals() for contains/remove checks

🧩 Keeps insertion order.

💬 Memory Trick:

“List just looks for equals.”

⸻


🎯 6️⃣ Quick Visual Mind Map
```
Collection
 ┣━━ List → allows duplicates, uses equals()
 ┃     ┣━━ ArrayList
 ┃     ┗━━ LinkedList
 ┣━━ Set → no duplicates
...
HashMap<K, V> is a key–value pair data structure in Java.
...
```
HashMap calls your key’s hashCode() method and mixes it with bitwise operations to reduce collisions.
Example:
```java
hash("Akshith") → 12345678
```
...
index = hash % capacity;
```
If the capacity is 16 → index = 12345678 % 16 = 14.

	3.	Go to that bucket
	
	•	If it’s empty, create a new Node and place it there.
	•	If it’s not empty, there’s a collision.
	
	4.	Handle collision
	
	•	HashMap uses LinkedList (before Java 8) or Balanced Tree (after Java 8).
	•	It iterates through the nodes in that bucket:
	•	If the key already exists (equals() true) → replace value.
	•	Else, add new node at the end.

⸻

🔹 4. When You Do map.get(K)
```java
map.get("Akshith");
```
	...
	•	For each node → check equals() on key.
...
🔹 5. Treeification (Java 8+ feature)
...
✅ Benefit → improves lookup time from O(n) to O(log n) in that bucket.
...
→ Resize happens when 16 × 0.75 = 12 entries

When resized:

	•	New capacity = old capacity × 2
	•	All existing entries are rehashed and moved to new buckets.

⸻

🔹 7. Key Methods Used Internally
```
...
Index 0 →  null
Index 1 →  [Node(hash=123, key="Akshith", value=100, next=null)]
Index 2 →  [Node(hash=456, key="Rahul", value=200, next=null)]
Index 3 →  null
Index 4 →  [Node(hash=789, key="Akshith", value=999, next=null)] ← collision
```

	•	Both “Akshith” entries go to different buckets only if hash differs.
	•	If same bucket, equals() decides whether to replace or append.

⸻

🔹 9. Common Interview Questions

✅ Why must you override both equals() and hashCode() for custom keys?
...
They go to the same bucket → HashMap uses equals() to differentiate them.

✅ What’s the time complexity?

	•	Best case: O(1)
	•	Worst case (all elements in one bucket): O(n)
	•	After Java 8 with tree: worst case O(log n)
...
✅ Features:
	...
	•	Faster than Hashtable (no synchronization)
	•	Backed by an array + linked list/tree (like we discussed before)
...
map.put("A", 10);
map.put(null, 20);
System.out.println(map.get(null)); // ✅ Works fine
```
...
✅ Features:
	...
	•	Slower because it locks the entire map for each operation
	•	Mostly deprecated for modern code

💡 Example:
```java
Map<String, Integer> table = new Hashtable<>();
table.put("A", 10);
// table.put(null, 20); ❌ Throws NullPointerException
```
...
✅ Features:
	...
	•	A wrapper around a normal HashMap
	•	Created using Collections.synchronizedMap()
	•	Makes all operations synchronized (like Hashtable)
	•	Still locks the entire map during each operation
	•	Slightly better alternative than Hashtable (modern class)
...
Even though it’s synchronized, you must still manually synchronize during iteration:
```java
...
→ multiple threads can read/write different parts at the same time

	•	Does NOT allow null keys or values
	•	Ideal for multi-threaded environments
...
// concurrentMap.put(null, 30); ❌ Throws NullPointerException
```
✅ High performance: because it doesn’t lock the whole map — only the specific bucket being modified.

⸻

🧠 6. Internal Difference (Locking Mechanism)
```
Map Type			Thread Safe	Locking Mechanism	Allows null		Performance
HashMap				❌ No		None					✅ Yes		🚀 Fastest (single-thread)
Hashtable			✅ Yes		Entire map				❌ No		🐢 Slow
SynchronizedMap		✅ Yes		Entire map				✅ Yes		🐢 Slow
ConcurrentHashMap	✅ Yes		Per-bucket (segment)	❌ No		⚡ Fast (multi-threaded)
```
...
Single-threaded environment			✅ HashMap
Legacy code (Java 1.1 or before)	☠️ Hashtable (avoid in new code)
Simple synchronization needed		⚙️ Collections.synchronizedMap()
Multi-threaded performance			⚡ ConcurrentHashMap
```
...
```
HashMap — Fast, not safe
Hashtable — Safe, but old and slow
SynchronizedMap — Safe wrapper, locks whole map
ConcurrentHashMap — Best of both worlds: safe & fast
```


# equals and HashCode contract (HashMap, HashSet, Hash...)

🔥 THE CONTRACT (Easy to Remember)
...
a.hashCode() == b.hashCode()  →  a.equals(b) could be true OR false

(Reason: hash collisions can happen.)

3. equals() must be consistent
	•	Must return the same result unless fields used for equality change.

4. hashCode() must be consistent
	•	Must return the same value unless fields used for equals change.
